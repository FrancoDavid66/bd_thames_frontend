...
export default function RenovacionesPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 p-3 md:p-6">
      ...
      {/* ============ RECIÉN VENCIDAS (≤3 días) ============ */}
      {recientes.length > 0 && (
        <div className="mb-4 rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm font-semibold text-amber-300">Recién vencidas · a tiempo de renovar</span>
            <span className="text-xs text-amber-200/80 bg-amber-500/15 px-2 py-0.5 rounded-full">{recientes.length}</span>
          </div>
          <p className="text-xs text-slate-400 mb-3">
            Finalizaron hace 3 días o menos. Renovalas antes de que pase el plazo (después serían póliza nueva).
          </p>
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm font-semibold text-white tracking-wide">{p.patente || "—"}</span>
                    <span className="text-xs text-slate-400 truncate">{p.compania_nombre || p.compania || ""}</span>
                  </div>
                  <div className="text-xs text-slate-500 truncate">
                    {p.cliente_nombre || p.asegurado_nombre || p.asegurado || ""}
                  </div>
                </div>
                <button
                  onClick={() => { setSelected(p); setModalOpen(true); }}
                  className="shrink-0 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-white text-xs font-semibold px-3 py-1.5 transition-colors"
                >
                  Renovar
                </button>
              </div>
