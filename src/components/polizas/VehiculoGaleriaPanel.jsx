...
const TIPOS = ["PATENTE","FRENTE","LATERAL_IZQ","LATERAL_DER","TRASERA","INTERIOR","EQUIPO_GNC","OTRA"];
...
export default function VehiculoGaleriaPanel({ polizaId }) {
  ...
  useEffect(() => { if (polizaId) load(); }, [polizaId]);
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {[...Array(6)].map((_,i)=>(
            <div key={i} className="h-36 bg-gray-800 rounded animate-pulse" />
          ))}
        </div>
      ) : fotos?.length ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {fotos.map((f) => (
            <div key={f.id} className="rounded-lg overflow-hidden border border-gray-700 bg-gray-800">
              ...
              <div className="p-2 text-xs flex items-center justify-between">
                <span className="text-gray-300 truncate">{f.tipo} · {f.origen}</span>
                <button onClick={()=>borrar(f.id)} className="text-red-300 hover:text-red-200">Eliminar</button>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="text-sm text-gray-400">No hay fotos cargadas para esta póliza.</div>
      )}
    </div>
  );
}
