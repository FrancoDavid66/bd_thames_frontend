...
export default function RenovarPolizaModal({ open, onClose, poliza }) {
  ...
  const options = useMemo(
    () => (Array.isArray(companias) ? companias : []).map((n) => ({ value: n, label: n })),
    [companias]
  );
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("poliza:renovada", { detail: { nuevaPoliza: data } }));
      onClose?.();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <form
        onSubmit={submit}
        className="relative w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900 text-slate-100 shadow-2xl overflow-hidden"
      >
        ...
        <div className="p-5">
          ...
          <p className="text-sm text-slate-300 mt-1">
            Se creará una <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">nueva póliza</span> para este mismo vehículo, con <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-pink-400">cuotas nuevas</span>. La póliza anterior quedará como historial.
          </p>
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full rounded-lg bg-slate-800/80 ring-1 ring-white/10 px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-400 placeholder:text-slate-500"
            ...
            <div>
              ...
              <div className="relative">
                <select
                  className="w-full appearance-none rounded-lg bg-slate-800/80 ring-1 ring-white/10 px-3 py-2 pr-9 outline-none focus:ring-2 focus:ring-sky-400 disabled:opacity-70"
                  value={nuevaCompania}
                  onChange={(e) => setNuevaCompania(e.target.value)}
                  disabled={loadingCompanias}
                >
                  <option value="">
                    {loadingCompanias ? "Cargando compañías..." : "— Sin cambio —"}
                  </option>
                  {options.map((opt) => (
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            <button
              type="button"
              className="px-3 py-2 rounded-lg border border-white/10 text-slate-200 hover:bg-white/10 transition"
              onClick={onClose}
              disabled={loading}
            >
              Cancelar
