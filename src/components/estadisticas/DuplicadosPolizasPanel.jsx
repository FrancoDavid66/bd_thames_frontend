  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-titulo dark:text-titulo-dark">Duplicados</h2>
          <p className="text-xs text-suave dark:text-suave-dark mt-0.5">Grupos de pólizas con datos repetidos — ideal para limpiar la base</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="h-9 bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark rounded-lg px-2.5 text-xs text-titulo dark:text-titulo-dark outline-none focus:border-oficina cursor-pointer dark:[color-scheme:dark]"
          >
        ...
        {resCards.map((c, i) => (
          <motion.button
            ...
            className={`rounded-xl border p-4 text-left transition-colors ${
              por === c.value
                ? "border-oficina bg-oficina/[0.08] ring-2 ring-oficina/15"
                : "border-linea dark:border-linea-dark bg-card dark:bg-card-dark hover:border-oficina/50"
            }`}
          >
            <div className={`text-[11px] mb-2 ${por === c.value ? "text-oficina" : "text-suave dark:text-suave-dark"}`}>
              {c.label}
            </div>
            <div className={`text-3xl font-semibold tabular-nums ${loadRes ? "text-suave dark:text-suave-dark" : por === c.value ? "text-oficina" : "text-titulo dark:text-titulo-dark"}`}>
              {loadRes ? "—" : c.v.toLocaleString("es-AR")}
            </div>
            <div className="text-[11px] text-suave dark:text-suave-dark mt-1">grupos duplicados</div>
          </motion.button>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-linea dark:border-linea-dark overflow-hidden bg-card dark:bg-card-dark">
        {/* Toolbar */}
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 border-b border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
          <div>
            <span className="text-xs font-medium text-titulo dark:text-titulo-dark">Grupos — {CRITERIOS.find(c => c.value === por)?.label}</span>
            <span className="ml-2 text-[11px] text-suave dark:text-suave-dark">{count.toLocaleString("es-AR")} grupos</span>
          </div>
          <div className="flex items-center gap-2">
            <select value={perGroup} onChange={e => setPerGroup(Number(e.target.value) || 12)}
              className="h-8 bg-card dark:bg-card-dark border border-linea dark:border-linea-dark rounded-md px-2 text-[11px] text-suave dark:text-suave-dark outline-none dark:[color-scheme:dark]">
        ...
        {/* Grupos */}
        <div className="divide-y divide-linea/40 dark:divide-linea-dark/40">
          {loadList ? (
            <div className="py-10 text-center text-xs text-suave dark:text-suave-dark">Cargando...</div>
          ) : grupos.length === 0 ? (
            <div className="py-12 text-center">
              <p className="text-sm text-ingreso font-medium">¡Sin duplicados!</p>
              <p className="text-xs text-suave dark:text-suave-dark mt-1">No hay grupos duplicados con este criterio.</p>
            </div>
          ) : grupos.map((g, idx) => {
            ...
            return (
              <div key={`${keyStr}-${idx}`} className="p-5">
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <div className="text-[11px] text-suave dark:text-suave-dark mb-0.5">Clave duplicada</div>
                    <div className="text-sm font-medium text-egreso">{keyStr || "—"}</div>
                    <div className="text-[11px] text-suave dark:text-suave-dark mt-0.5">{Number(g?.count || items.length || 0)} pólizas</div>
                  </div>
                  <div className="flex items-center gap-2">
                    {items.filter(it => String(it.estado) === "activa").length > 1 && (
                      <button onClick={() => abrirResolver(items)}
                        className="flex items-center gap-1.5 h-8 px-2.5 rounded-lg bg-tarjeta text-white font-medium hover:brightness-105 transition-colors text-[11px]">
                        Resolver
                      </button>
                    )}
                    <button onClick={() => copyText(keyStr)}
                      className="flex items-center gap-1.5 h-8 px-2.5 rounded-lg border border-linea dark:border-linea-dark text-suave dark:text-suave-dark hover:border-oficina hover:text-oficina transition-colors text-[11px] font-medium">
                ...
                <div className="rounded-lg border border-linea dark:border-linea-dark overflow-hidden">
                  <table className="min-w-full text-xs">
                    <thead>
                      <tr className="border-b border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
                        <th className="px-3 py-2 text-left text-[11px] text-suave dark:text-suave-dark">Póliza</th>
                        <th className="px-3 py-2 text-left text-[11px] text-suave dark:text-suave-dark">Cliente</th>
                        <th className="px-3 py-2 text-left text-[11px] text-suave dark:text-suave-dark">Patente</th>
                        <th className="px-3 py-2 text-left text-[11px] text-suave dark:text-suave-dark">Oficina</th>
                        <th className="px-3 py-2 text-left text-[11px] text-suave dark:text-suave-dark">Estado</th>
                        <th className="px-3 py-2 text-right"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-linea/40 dark:divide-linea-dark/40">
                      {items.length === 0 ? (
                        <tr><td colSpan={6} className="px-3 py-4 text-center text-suave dark:text-suave-dark text-[11px]">Sin ítems. Aumentá "por grupo".</td></tr>
                      ) : items.map(it => (
                        <tr key={it.id} className="hover:bg-oficina/5 transition-colors group">
                          <td className="px-3 py-2.5">
                            <button onClick={() => navigate(`/polizas/${it.id}`)} className="font-medium text-oficina hover:text-oficina-fuerte transition-colors text-left">
                              #{it.id}{it.numero_poliza ? ` · ${it.numero_poliza}` : ""}
                            </button>
                            {it.compania && <div className="text-[11px] text-suave dark:text-suave-dark">{it.compania}</div>}
                          </td>
                          <td className="px-3 py-2.5">
                            <div className="text-titulo dark:text-titulo-dark">{it?.cliente?.nombre || "—"}</div>
                            {it?.cliente?.dni_cuit_cuil && <div className="text-[11px] text-suave dark:text-suave-dark">{it.cliente.dni_cuit_cuil}</div>}
                          </td>
                          <td className="px-3 py-2.5 text-suave dark:text-suave-dark">{it.patente || "—"}</td>
                          <td className="px-3 py-2.5 text-suave dark:text-suave-dark">{getOficinaNombre ? (getOficinaNombre(it.oficina) || "—") : (it.oficina ?? "—")}</td>
                          <td className="px-3 py-2.5">
        ...
        {totalPagesN > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
            <span className="text-[11px] text-suave dark:text-suave-dark">Página {page} de {totalPagesN}</span>
      ...
      {resolverItems && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-lg bg-card dark:bg-card-dark border border-linea dark:border-linea-dark rounded-xl shadow-xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-linea dark:border-linea-dark">
              <h3 className="text-base font-semibold text-titulo dark:text-titulo-dark">Resolver duplicado</h3>
              <p className="text-xs text-suave dark:text-suave-dark mt-0.5">Elegí la póliza que queda <b className="text-ingreso">activa</b>. Las demás pasan a <b className="text-titulo dark:text-titulo-dark">finalizada</b> (no se borran).</p>
            </div>
            ...
            <div className="px-5 py-4 space-y-2 max-h-[50vh] overflow-y-auto">
              {resolverItems.map(it => {
                ...
                return (
                  <label key={it.id}
                    className={`flex items-start gap-3 rounded-xl border p-3 cursor-pointer transition-colors ${sel ? "border-ingreso bg-ingreso/10" : "border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark hover:border-ingreso/50"}`}>
                    <input type="radio" name="activa" checked={sel} onChange={() => setResolverActiva(it.id)} className="mt-1 accent-[var(--color-ingreso)]" />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-titulo dark:text-titulo-dark font-medium">#{it.id}{it.numero_poliza ? ` · ${it.numero_poliza}` : ""}</div>
