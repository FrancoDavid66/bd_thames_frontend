...
const inputCls = "h-9 rounded-lg bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark px-2.5 text-xs text-titulo dark:text-titulo-dark outline-none focus:border-oficina placeholder:text-suave dark:placeholder:text-suave-dark transition-colors dark:[color-scheme:dark]";

function SmallInput({ label, icon: Icon, ...props }) {
  return (
    <div className="flex flex-col gap-1">
      {label && <label className="text-[11px] text-suave dark:text-suave-dark flex items-center gap-1">{Icon && <Icon className="text-xs" />}{label}</label>}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-base font-semibold text-titulo dark:text-titulo-dark flex items-center gap-2"><HiTruck className="text-oficina text-sm"/>Análisis de vehículos</h2>
          <p className="text-xs text-suave dark:text-suave-dark mt-0.5">Filtrá por tipo, año, marca o patente</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => { fetchResumen(); fetchList(); }} disabled={loading} className="h-9 w-9 flex items-center justify-center rounded-lg border border-linea dark:border-linea-dark text-suave dark:text-suave-dark hover:border-oficina hover:text-oficina transition-colors">
            <HiRefresh className={`text-sm ${loading ? "animate-spin" : ""}`} />
          </button>
          <button onClick={() => onOpenExport?.(filtros)} className="h-9 flex items-center gap-1.5 px-3.5 rounded-lg border border-linea dark:border-linea-dark text-titulo dark:text-titulo-dark hover:border-oficina hover:text-oficina transition-colors text-xs font-medium">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-suave dark:text-suave-dark flex items-center gap-1"><HiOfficeBuilding className="text-xs"/>Oficina</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-suave dark:text-suave-dark flex items-center gap-1"><HiTruck className="text-xs"/>Tipo</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-[11px] text-suave dark:text-suave-dark opacity-0 select-none">·</label>
      ...
      <div className="flex flex-wrap gap-3">
        <div className="rounded-xl border border-linea dark:border-linea-dark bg-card dark:bg-card-dark px-4 py-2.5 flex items-baseline gap-2">
          <span className="text-2xl font-semibold text-titulo dark:text-titulo-dark tabular-nums">{loadRes ? "—" : total.toLocaleString("es-AR")}</span>
          <span className="text-[11px] text-suave dark:text-suave-dark">pólizas</span>
        </div>
        <div className="rounded-xl border border-ingreso/25 bg-ingreso/[0.06] px-4 py-2.5 flex items-baseline gap-2">
          <span className="text-2xl font-semibold text-ingreso tabular-nums">{loadRes ? "—" : activas.toLocaleString("es-AR")}</span>
          <span className="text-[11px] text-suave dark:text-suave-dark">activas</span>
        </div>
        {oficina && (
          <div className="rounded-xl border border-linea dark:border-linea-dark bg-card dark:bg-card-dark px-4 py-2.5 flex items-center gap-1.5">
            <HiOfficeBuilding className="text-oficina text-xs" />
            <span className="text-xs font-medium text-titulo dark:text-titulo-dark">{getOficinaNombre(oficina)}</span>
          </div>
        )}
      </div>
      ...
      {resumen && (
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-linea dark:border-linea-dark bg-card dark:bg-card-dark p-4 space-y-3">
            <h3 className="text-[11px] text-suave dark:text-suave-dark">Por tipo</h3>
            {topTipos.length === 0 ? <p className="text-xs text-suave dark:text-suave-dark">Sin datos</p> : topTipos.map(([k,v]) => (
              <div key={k} className="flex items-center gap-2">
                <span className="text-xs text-titulo dark:text-titulo-dark w-20 truncate">{k}</span>
                <MiniBar value={Number(v)} max={maxTipo} />
                <span className="text-xs font-medium text-suave dark:text-suave-dark tabular-nums w-10 text-right">{Number(v).toLocaleString("es-AR")}</span>
              </div>
            ))}
          </div>
          <div className="rounded-xl border border-linea dark:border-linea-dark bg-card dark:bg-card-dark p-4 space-y-3">
            <h3 className="text-[11px] text-suave dark:text-suave-dark">Por año</h3>
            {topAnios.length === 0 ? <p className="text-xs text-suave dark:text-suave-dark">Sin datos</p> : topAnios.map(([k,v]) => (
              <div key={k} className="flex items-center gap-2">
                <span className="text-xs text-titulo dark:text-titulo-dark w-12 tabular-nums">{k}</span>
      ...
      <div className="rounded-xl border border-linea dark:border-linea-dark overflow-hidden bg-card dark:bg-card-dark">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
          <span className="text-xs font-medium text-titulo dark:text-titulo-dark">Asegurados</span>
          <span className="text-[11px] text-suave dark:text-suave-dark">{loadList ? "Cargando..." : `${Number(listData.count).toLocaleString("es-AR")} · pág ${page}/${listData.total_pages}`}</span>
        </div>
        {listData.count === 0 && !loadList ? (
          <div className="py-10 text-center text-xs text-suave dark:text-suave-dark">Sin resultados con estos filtros.</div>
        ) : (
          <>
              ...
              <table className="min-w-full text-xs">
                <thead>
                  <tr className="border-b border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
                    ...
                    <th className="px-4 py-2.5 text-left text-[11px] text-suave dark:text-suave-dark">Vehículo</th>
                    <th className="px-4 py-2.5 text-right text-[11px] text-suave dark:text-suave-dark">Año</th>
                    <th className="px-4 py-2.5 text-left text-[11px] text-suave dark:text-suave-dark">Oficina</th>
                ...
                <tbody className="divide-y divide-linea/50 dark:divide-linea-dark/50">
                  {loadList ? (
                    <tr><td colSpan={5} className="py-8 text-center text-suave dark:text-suave-dark">Cargando...</td></tr>
                  ) : listData.results.map((r, idx) => (
                    <tr key={`${r.poliza_id}-${idx}`} className="hover:bg-oficina/5 transition-colors">
                      <td className="px-4 py-3">
                        <div className="font-medium text-titulo dark:text-titulo-dark">{r.asegurado || "—"}</div>
                        <div className="text-[11px] text-suave dark:text-suave-dark mt-0.5 font-mono">{r.patente || "—"} · {r.numero_poliza || "—"}</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-titulo dark:text-titulo-dark">{[r.marca, r.modelo].filter(Boolean).join(" ") || "—"}</div>
                        <div className="text-[11px] text-suave dark:text-suave-dark">{r.tipo || "—"}</div>
                      </td>
                      <td className="px-4 py-3 text-right font-mono text-suave dark:text-suave-dark tabular-nums">{r.anio ?? "—"}</td>
                      <td className="px-4 py-3 text-suave dark:text-suave-dark text-[11px]">{r.oficina_nombre || getOficinaNombre(r.oficina) || "—"}</td>
                      <td className="px-4 py-3">
                        <span className={`text-[11px] ${r.estado === "activa" ? "text-ingreso" : "text-suave dark:text-suave-dark"}`}>{r.estado || "—"}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
              <span className="text-[11px] text-suave dark:text-suave-dark">{listData.results.length} de {Number(listData.count).toLocaleString("es-AR")}</span>
              <div className="flex items-center gap-1.5">
                <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={!canPrev} className="h-8 w-8 flex items-center justify-center rounded-lg border border-linea dark:border-linea-dark text-suave dark:text-suave-dark hover:border-oficina hover:text-oficina disabled:opacity-30 transition-colors"><HiChevronLeft className="text-xs"/></button>
                <button onClick={() => setPage(p => p+1)} disabled={!canNext} className="h-8 w-8 flex items-center justify-center rounded-lg border border-linea dark:border-linea-dark text-suave dark:text-suave-dark hover:border-oficina hover:text-oficina disabled:opacity-30 transition-colors"><HiChevronRight className="text-xs"/></button>
              </div>
            </div>
          </>
        )}
