...
export default function RegistrarPagoModal({ pago, onClose, onSuccess }) {
  ...
  const inputCls = "w-full px-3 h-11 rounded-lg bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark focus:border-oficina focus:outline-none text-sm text-titulo dark:text-titulo-dark disabled:opacity-60 transition-colors dark:[color-scheme:dark]";
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* HEADER */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-linea dark:border-linea-dark shrink-0">
            ...
            <button
              onClick={onClose}
              className="ml-3 w-10 h-10 rounded-lg bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark hover:border-egreso hover:text-egreso flex items-center justify-center text-suave dark:text-suave-dark transition-colors shrink-0"
            >
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
            {yaPagado ? (
              ...
              <>
                <div className="flex items-center gap-2 p-3 rounded-xl bg-ingreso/10 border border-ingreso/30">
                  <HiCheck className="w-5 h-5 text-ingreso shrink-0" />
                  <p className="text-sm text-ingreso dark:text-ingreso-claro font-medium">
                    Pagado el {dayjs(pago.fecha_pago).format("DD/MM/YYYY")}
                  </p>
                </div>

                <Field label="Monto" icon={<HiOutlineCash className="w-3.5 h-3.5" />}>
                  <div className="flex items-center gap-2 w-full px-4 h-14 border border-linea dark:border-linea-dark rounded-xl bg-surface dark:bg-surface-dark">
                    <span className="text-2xl font-semibold text-suave dark:text-suave-dark">$</span>
                    <input type="text" value={montoToDisplay(monto)} disabled className="flex-1 min-w-0 bg-transparent outline-none text-2xl font-mono font-semibold text-titulo dark:text-titulo-dark disabled:opacity-60" />
                  </div>
                </Field>
                ...
                <Field label="Forma de pago" icon={<HiOutlineCreditCard className="w-3.5 h-3.5" />}>
                  <div className="h-11 flex items-center px-3 rounded-lg bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark text-sm text-titulo dark:text-titulo-dark">
                ...
                <Field label="Comprobante" icon={<HiOutlineReceiptTax className="w-3.5 h-3.5" />}>
                  {comprobanteUrl ? (
                    <div className="space-y-2">
                      <div className="relative rounded-xl overflow-hidden border border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
                        {comprobanteUrl.toLowerCase().endsWith(".pdf") ? (
                          <a href={comprobanteUrl} target="_blank" rel="noreferrer" className="block p-6 text-center hover:bg-oficina/5 transition-colors">
                            <HiOutlineDocumentText className="w-8 h-8 mx-auto text-suave dark:text-suave-dark mb-1" />
                            <p className="text-xs font-medium text-titulo dark:text-titulo-dark">Abrir PDF</p>
                          </a>
                        ) : (
                          <a href={comprobanteUrl} target="_blank" rel="noreferrer" title="Ver en grande">
                            <img src={comprobanteUrl} alt="Comprobante" className="w-full max-h-64 object-contain hover:opacity-90 transition-opacity" />
                          </a>
                        )}
                      </div>
                      <div className="flex gap-2">
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex-1 h-10 inline-flex items-center justify-center gap-1.5 rounded-lg border border-linea dark:border-linea-dark text-sm font-medium text-titulo dark:text-titulo-dark hover:border-oficina hover:text-oficina transition-colors"
                        >
                        ...
                        <a
                          ...
                          className="flex-1 h-10 inline-flex items-center justify-center gap-1.5 rounded-lg bg-oficina text-white text-sm font-medium hover:brightness-110 transition-colors"
                        >
                    ...
                    <p className="text-xs text-suave dark:text-suave-dark">Sin comprobante</p>
                  )}
                </Field>

                {observaciones && (
                  <Field label="Notas">
                    <p className="text-sm text-titulo dark:text-titulo-dark">{observaciones}</p>
                  </Field>
                )}
              </>
            ) : (
              ...
              <>
                ...
                <div className="flex items-center gap-2">
                  {PASOS.map((p, i) => (
                    <Fragment key={p.n}>
                      <div className="flex items-center gap-2">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium border transition-colors ${step >= p.n ? "bg-oficina text-white border-oficina" : "bg-surface dark:bg-surface-dark text-suave dark:text-suave-dark border-linea dark:border-linea-dark"}`}>
                          {p.n}
                        </div>
                        <span className={`text-xs hidden sm:inline ${step >= p.n ? "text-titulo dark:text-titulo-dark font-medium" : "text-suave dark:text-suave-dark"}`}>{p.label}</span>
                      </div>
                      {i < PASOS.length - 1 && (
                ...
                {step === 1 && (
                  <div className="space-y-4">
                    <Field label="Monto" icon={<HiOutlineCash className="w-3.5 h-3.5" />}>
                      <div className="flex items-center gap-2 w-full px-4 h-14 border border-linea dark:border-linea-dark rounded-xl bg-surface dark:bg-surface-dark focus-within:border-oficina transition-colors">
                        <span className="text-2xl font-semibold text-suave dark:text-suave-dark">$</span>
                        <input
                          type="text"
                          inputMode="decimal"
                          ...
                          placeholder="0"
                          autoFocus
                          className="flex-1 min-w-0 bg-transparent outline-none text-2xl font-mono font-semibold text-titulo dark:text-titulo-dark placeholder:text-suave dark:placeholder:text-suave-dark"
                        />
                      </div>
                    </Field>
                ...
                {step === 2 && (
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <button
                            ...
                            className={`h-11 rounded-lg text-xs font-medium transition-colors border ${
                              formaPago === opt.v
                    ...
                    {formaPago !== "EFECTIVO" && (
                      <Field label="Cuenta">
                        ...
                        {mediosCobro.length === 0 && (
                          <p className="text-xs text-tarjeta mt-1">
                            No hay billeteras configuradas. Cargá una desde Configuración de Pagos.
                          </p>
                        )}
                      </Field>
                    )}
                    ...
                    {formaPago === "EFECTIVO" && (
                      <p className="text-[11px] text-suave dark:text-suave-dark italic">
                ...
                {step === 3 && (
                  <div className="space-y-4">
                    <Field
                      label={<>Comprobante <span className="text-suave dark:text-suave-dark font-normal normal-case">(opcional)</span></>}
                      ...
                      {comprobanteUrl ? (
                        <div className="relative rounded-xl overflow-hidden border border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark">
                          {comprobanteFile?.type === "application/pdf" || comprobanteUrl?.endsWith?.(".pdf") ? (
                            <a href={comprobanteUrl} target="_blank" rel="noreferrer" className="block p-6 text-center hover:bg-oficina/5 transition-colors">
                              <HiOutlineDocumentText className="w-8 h-8 mx-auto text-suave dark:text-suave-dark mb-1" />
                              <p className="text-xs font-medium text-titulo dark:text-titulo-dark">Ver PDF</p>
                            </a>
                          ) : (
                          ...
                          <button
                            ...
                            className="absolute top-2 right-2 w-8 h-8 rounded-full bg-egreso hover:bg-egreso-fuerte flex items-center justify-center text-white transition-colors"
                          >
                        ...
                        <label className="block cursor-pointer">
                          ...
                          <div className="rounded-xl border-2 border-dashed border-linea dark:border-linea-dark hover:border-oficina hover:bg-oficina/5 px-4 py-6 text-center transition-colors">
                            <HiOutlineCloudUpload className="w-8 h-8 mx-auto text-suave dark:text-suave-dark mb-1.5" />
                            <p className="text-xs font-medium text-titulo dark:text-titulo-dark">Subir comprobante</p>
                            <p className="text-[10px] text-suave dark:text-suave-dark mt-0.5">Foto o PDF</p>
                          </div>
                        </label>
                      )}
                    </Field>
                    ...
                    <Field label="Notas (opcional)">
                      <textarea
                        ...
                        rows={2}
                        placeholder="..."
                        className="w-full px-3 py-2 rounded-lg bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark focus:border-oficina focus:outline-none resize-none text-sm text-titulo dark:text-titulo-dark transition-colors"
                      />
                    </Field>

                    {/* Resumen */}
                    <div className="rounded-xl border border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark p-3 text-sm space-y-1.5">
                      <p className="text-[11px] text-suave dark:text-suave-dark mb-1">Resumen</p>
                      <div className="flex justify-between"><span className="text-suave dark:text-suave-dark">Monto</span><span className="font-mono font-semibold text-titulo dark:text-titulo-dark">{fmt(monto)}</span></div>
                      <div className="flex justify-between"><span className="text-suave dark:text-suave-dark">Fecha</span><span className="text-titulo dark:text-titulo-dark">{dayjs(fecha).format("DD/MM/YYYY")}</span></div>
                      <div className="flex justify-between"><span className="text-suave dark:text-suave-dark">Forma de pago</span><span className="text-titulo dark:text-titulo-dark">{labelForma(formaPago)}</span></div>
                      {formaPago !== "EFECTIVO" && (
                        <div className="flex justify-between"><span className="text-suave dark:text-suave-dark">Cuenta</span><span className="text-titulo dark:text-titulo-dark">{cuentaLabel || "—"}</span></div>
                      )}
                    </div>
                  </div>
                )}
          ...
          {/* FOOTER */}
          <div className="px-5 py-4 border-t border-linea dark:border-linea-dark shrink-0 bg-card dark:bg-card-dark">
            {yaPagado ? (
              <button
                type="button"
                onClick={handleDeshacer}
                disabled={submitting || deshaciendo}
                className="w-full h-12 rounded-xl bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark hover:border-egreso hover:bg-egreso/10 text-suave dark:text-suave-dark hover:text-egreso font-medium text-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {deshaciendo ? (
                  <>
              ...
              <div className="flex gap-2">
                {step === 1 ? (
                  <button type="button" onClick={onClose} className="flex-1 h-12 rounded-xl bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark hover:border-egreso hover:text-egreso text-titulo dark:text-titulo-dark font-medium text-sm transition-colors">
                    Cancelar
                  </button>
                ) : (
                  <button type="button" onClick={atras} className="flex-1 h-12 rounded-xl bg-surface dark:bg-surface-dark border border-linea dark:border-linea-dark hover:border-oficina text-titulo dark:text-titulo-dark font-medium text-sm transition-colors">
                    ← Atrás
                  </button>
                )}

                {step < 3 ? (
                  <button type="button" onClick={siguiente} className="flex-1 h-12 rounded-xl bg-oficina text-white font-medium text-sm hover:brightness-110 transition-colors">
                    Siguiente →
                  </button>
                ) : (
                  <button
                    ...
                    className="flex-1 h-12 rounded-xl bg-ingreso text-white font-medium text-sm hover:brightness-110 transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {subiendo || submitting ? (
                      <>
...
function Field({ label, icon, children }) {
  return (
    <div>
      <label className="flex items-center gap-1 text-[11px] text-suave dark:text-suave-dark mb-1.5">
        {icon}
        <span>{label}</span>
      </label>
      {children}
