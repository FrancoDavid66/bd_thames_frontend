...
/* =====================  Constantes  ===================== */
// si no está definido, no bloqueamos carga (12 es buen default)
const MAX_FOTOS_RAW = import.meta.env.VITE_MAX_FOTOS;
const MAX_FOTOS =
  MAX_FOTOS_RAW === "0" || MAX_FOTOS_RAW === 0
    ? 0
    : Number(MAX_FOTOS_RAW || 12);
...
// ===== Slots (alineados a backend TipoDocSolicitud) =====

// RC pura: cédula frente requerida, dorso opcional
const DOC_SLOT_RC = [
  { key: "CEDULA_VERDE_FRENTE", label: "Cédula verde (frente)" },
  { key: "CEDULA_VERDE_DORSO", label: "Cédula verde (dorso)" },
];

// A + GRÚA: mismos docs (por ahora sin VTV porque backend no lo tiene)
const DOC_SLOT_A_GRUA = [
  { key: "CEDULA_VERDE_FRENTE", label: "Cédula verde (frente)" },
  { key: "CEDULA_VERDE_DORSO", label: "Cédula verde (dorso)" },
];

// Full: docs reales del enum (sin VTV / PERMISO)
const DOC_SLOT_FULL = [
  { key: "CEDULA_VERDE_FRENTE", label: "Cédula verde (frente)" },
  { key: "CEDULA_VERDE_DORSO", label: "Cédula verde (dorso)" },
...
// Fotos del vehículo (solo keys reales del backend en solicitudes)
const FOTO_SLOTS_FULL = [
  ...
  { key: "TUBO_GNC", label: "Equipo GNC" }, // se mapea a EQUIPO_GNC
];
...
// ✅ claves de fotos admitidas por backend (solicitud)
...
export default function CreateSolicitudModal({
  ...
  // ✅ submit solo si responsable + paso1 + paso2 OK
  ...
  const onSubmit = async () => {
    if (!canSubmit) {
      if (!responsableOk) toast.error("Falta responsable");
      else if (!canNext1) toast.error("Revisá cliente");
      else if (!canNext2) toast.error("Revisá póliza");
      return;
    }
    ...
    try {
      ...
      // ===== DOCUMENTOS DEL VEHÍCULO (solo slots válidos)
      ...
      if (import.meta?.env?.DEV) console.debug("[CREAR_COMPLETO][payload]", payload);
      ...
      if (typeof onCreated === "function") {
        onCreated({
          primera_cuota_id:
            ...
            null,
          telefono: raw?.telefono ?? raw?.cliente?.telefono ?? raw?.poliza?.cliente?.telefono ?? "",
          precio_cuota: raw?.precio_cuota ?? raw?.poliza?.precio_cuota ?? "",
  ...
  return (
    <div className="fixed inset-0 z-[90] overscroll-contain touch-pan-y">
      ...
      <motion.div
        ...
        animate="animate"
        exit="exit"
        className="relative w-full max-w-md sm:max-w-5xl lg:max-w-6xl h-[90vh] sm:h-auto mx-auto rounded-2xl border border-white/10 shadow-[0_20px_80px_rgba(0,0,0,.5)] flex flex-col overflow-hidden"
        ...
        <div className="sticky top-0 z-10 border-b border-white/10 bg-[#0f0c28]/80 backdrop-blur" style={{ paddingTop: "calc(env(safe-area-inset-top) + 2px)" }}>
          <div className="flex items-center justify-between px-3 md:px-4 py-2 md:py-3">
            <h3 className="text-white font-semibold flex items-center gap-2 text-base md:text-lg">
              <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-rose-200/80 to-violet-200/80 text-[#0b0f1e] shadow">
                <HiSparkles />
              </span>
              Nueva solicitud
            </h3>
            ...
            <div className="flex items-center gap-2">
              {Boolean(String(responsableId).trim()) && (
                <span className="hidden lg:inline-flex items-center gap-2 text-[11px] px-2 py-1 rounded-lg border border-white/15 text-white/90 bg-white/5">
                  ...
                  <button
                    type="button"
                    onClick={() => setAskResponsable(true)}
                    className="ml-1 text-white/90 underline underline-offset-2 hover:text-white"
              ...
              <motion.button
                ...
                onClick={() => !saving && onClose?.()}
                disabled={saving}
                className="inline-flex items-center justify-center h-10 w-10 rounded-xl bg-white/10 text-white transition hover:bg-white/20 disabled:opacity-50"
                aria-label="Cerrar"
        ...
        <div className="sticky bottom-0 z-30 border-t border-white/10 bg-[#0f0c28]/80 backdrop-blur" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
          <div className="flex items-center justify-between gap-2 px-3 md:px-4 py-3">
            <motion.button
              onClick={() => !saving && onClose?.()}
              disabled={saving}
              className="px-4 py-3 rounded-2xl bg-white/10 text-sm md:text-base text-white transition w-[44%] md:w-auto hover:bg-white/20 disabled:opacity-60"
              variants={buttonVariants}
              initial="initial"
              whileHover="hover"
              whileTap="tap"
            ...
            <div className="flex items-center gap-2 w-[56%] md:w-auto">
              {step > 1 && (
                <motion.button
                  ...
                  disabled={saving}
                  className="flex-1 md:flex-none px-4 py-3 rounded-2xl bg-white/10 text-sm md:text-base text-white inline-flex items-center justify-center gap-2 transition hover:bg-white/20"
                  variants={buttonVariants}
                  initial="initial"
                  whileHover="hover"
                  whileTap="tap"
              ...
              {step < 4 && (
                <motion.button
                  ...
                  disabled={saving}
                  className="flex-1 md:flex-none px-4 py-3 rounded-2xl text-[#0b0f1e] font-semibold bg-gradient-to-br from-sky-200 to-cyan-200 shadow-lg inline-flex items-center justify-center gap-2 transition text-sm md:text-base hover:brightness-105 disabled:opacity-60"
                  variants={buttonVariants}
                  initial="initial"
                  whileHover="hover"
                  whileTap="tap"
              ...
              {step === 4 && (
                <motion.button
                  onClick={onSubmit}
                  disabled={!canSubmit}
                  className="flex-1 md:flex-none px-4 py-3 rounded-2xl text-[#0b0f1e] font-semibold bg-gradient-to-br from-emerald-200 to-teal-200 shadow-lg inline-flex items-center justify-center gap-2 transition text-sm md:text-base hover:brightness-105 disabled:opacity-60"
                  variants={buttonVariants}
                  initial="initial"
                  whileHover="hover"
                  whileTap="tap"
  ...
  return (
    <motion.button
      ...
      className={`flex items-center gap-2 rounded-xl px-3 py-2 border text-sm transition min-w-[140px] snap-center ${
        active ? "border-white/20 bg-gradient-to-br from-violet-200/80 to-indigo-200/80 text-[#0b0f1e]" : "border-white/10 bg-white/5 text-white/85"
      }`}
    ...
    >
      <span
        className={`inline-flex items-center justify-center w-6 h-6 rounded-md ${
          active ? "bg-black/10 text-black" : done ? "bg-emerald-300/30 text-emerald-200" : "bg-white/10 text-white"
        }`}
      >
  ...
  return (
    <motion.button
      ...
      className={`flex items-center gap-2 rounded-xl px-3 py-2 border w-full text-left transition ${
        active ? "border-white/20 bg-gradient-to-br " + color + " text-[#0b0f1e]" : "border-white/10 bg-white/5 text-white/85 hover:bg-white/10"
      }`}
    >
      <span
        className={`inline-flex items-center justify-center w-6 h-6 rounded-md ${
