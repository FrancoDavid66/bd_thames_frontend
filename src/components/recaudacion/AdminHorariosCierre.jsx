...
const INPUT =
  "w-full rounded-lg border border-linea dark:border-linea-dark bg-surface dark:bg-surface-dark " +
  "px-3 py-2.5 text-sm text-titulo dark:text-titulo-dark outline-none " +
      ...
      <div className="divide-y divide-linea dark:divide-linea-dark">
        {filas.map((f, i) => (
          <div key={f.oficina} className="px-5 py-6">
            <div className="mb-4 flex items-center gap-2 text-[15px] font-semibold text-titulo dark:text-titulo-dark">
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-oficina px-4 py-2.5 text-sm font-medium text-white hover:brightness-110 transition-colors disabled:opacity-50"
            >
              {guardando === f.oficina
...
function Campo({ label, children }) {
  return (
    <div>
      <label className="mb-1.5 block text-[11px] text-suave dark:text-suave-dark">
        {label}
      </label>
      {children}
    </div>
