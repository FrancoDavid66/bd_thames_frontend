// src/components/pagos/RecorditoriosCuotasModal.jsx
...
export default function RecorditoriosCuotasModal({
  isOpen,
  onClose,
  mediosCobro = [],
  sending = false,
  onEnviar, // (medio_cobro_id | null) => void|Promise
}) {
  ...
  const handleConfirm = async () => {
    if (!onEnviar) return;
    await onEnviar(selectedId || null);
  };

  const canConfirm = !sending && (selectedId || mediosAptos.length === 0);
              ...
              <Dialog.Panel className="w-full max-w-lg rounded-3xl bg-neutral-950 border border-neutral-800 ring-1 ring-neutral-800/80 text-white shadow-2xl">
                ...
                <div className="px-6 pb-5 pt-2 flex items-center justify-between gap-3 border-t border-neutral-800">
                  <p className="text-[11px] text-neutral-400 max-w-xs">
                    Se enviará un solo WhatsApp por cliente agrupando sus cuotas. El alias
                    elegido aparecerá como medio de pago por transferencia.
                  </p>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
