...
import { HiReceiptTax, HiUser, HiOfficeBuilding, HiCash, HiCalendar } from "react-icons/hi";
    ...
    <div className="max-w-3xl mx-auto rounded-2xl border border-neutral-300/20 bg-neutral-500 text-neutral-100 shadow print:bg-white print:text-black print:border-none print:shadow-none">
      ...
      <div className="rounded-t-2xl bg-primary-400 text-neutral-900 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <h1 className="text-xl font-bold">Factura / Recibo de Pago de Cuota</h1>
            <p className="text-xs opacity-80">Emitida el {fmtDate(new Date())}</p>
          </div>
        </div>
        <div className="text-right">
          <p className="text-xs opacity-70">Nº de comprobante</p>
          <p className="text-base font-semibold">{String(cuota.id).padStart(6, "0")}</p>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6">
          ...
          <section className="rounded-xl border border-neutral-300/20 bg-neutral-400/30 p-4">
            ...
            <ul className="text-sm space-y-1">
              ...
              <li>
                ...
                <span className="font-medium">{safe(cliente.dni_cuit_cuil)}</span>
              </li>
              <li>
          ...
          <section className="rounded-xl border border-neutral-300/20 bg-neutral-400/30 p-4">
            ...
            <ul className="text-sm space-y-1">
              <li>
                ...
                <span className="font-medium">{safe(poliza.numero_poliza)}</span>
              </li>
              <li>
              ...
              <li>
                ...
                <span className="font-medium">
                  {safe(poliza.marca)} {safe(poliza.modelo)} {poliza.anio ? `(${poliza.anio})` : ""}
                </span>
              </li>
              <li>
        ...
        <section className="rounded-xl border border-neutral-300/20 bg-neutral-400/30 p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
            ...
            <div className="rounded-lg border border-neutral-300/20 bg-neutral-500/20 p-3">
              ...
              <p className="text-lg font-semibold">{fmtMoney(cuota.monto)}</p>
            </div>
            <div className="rounded-lg border border-neutral-300/20 bg-neutral-500/20 p-3">
              <p className="opacity-80">Estado</p>
              <p className="text-lg font-semibold">{cuota.pagado ? "Pagada" : "Pendiente"}</p>
            </div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
            <div className="rounded-lg border border-neutral-300/20 bg-neutral-500/20 p-3">
              ...
              <p className="font-medium">{fmtDate(cuota.fecha_vencimiento)}</p>
            </div>
            <div className="rounded-lg border border-neutral-300/20 bg-neutral-500/20 p-3">
              <p className="flex items-center gap-1 opacity-80">
              ...
              <p className="font-medium">
                {cuota.pagado ? fmtDate(cuota.fecha_pago || new Date()) : "—"}
              </p>
            </div>
            <div className="rounded-lg border border-neutral-300/20 bg-neutral-500/20 p-3">
        ...
        <div className="text-center text-xs text-neutral-200/70 print:text-neutral-600">
          ...
          <br />
          <span className="opacity-70">STARKE | Sistema de Gestión de Seguros</span>
        </div>
      </div>
    </div>
  );
