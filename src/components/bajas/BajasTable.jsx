  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-900/50 backdrop-blur-md overflow-hidden shadow-2xl">
      <div className="grid grid-cols-12 gap-2 px-6 py-4 bg-white/5 text-[11px] font-bold text-white/50 uppercase tracking-widest border-b border-white/5">
        ...
        <div className="col-span-2 cursor-pointer hover:text-white transition-colors" onClick={() => onSort?.('max_vto_impaga')}>
          Últ. cuota vencida <SortIcon columnKey="max_vto_impaga" />
        </div>
          ...
          {rows.map((p) => {
            ...
            return (
              <div key={p.id} className={`grid grid-cols-12 gap-2 px-6 py-5 transition-all items-center ${isSelected ? "bg-sky-500/10" : "hover:bg-white/[0.03]"}`}>
                ...
                <div className="col-span-2">
                  {/* Última cuota vencida — la más relevante para decidir la baja */}
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <a
                      href={`/polizas/${p.id}`}
                      className="inline-flex items-center gap-1 text-[10px] text-sky-400/70 hover:text-sky-400 transition-colors font-bold uppercase tracking-wider"
                    >
                      Ver ficha <HiExternalLink />
                    </a>
                    {/* Nueva póliza — cuando el cliente viene a pagar */}
                    <a
                      href={`/solicitudes?nueva=1&cliente=${p?.cliente?.id || p?.cliente_id || ""}&patente=${p?.patente || ""}`}
                      className="inline-flex items-center gap-1 text-[10px] text-emerald-400/70 hover:text-emerald-400 transition-colors font-bold uppercase tracking-wider"
                      title="El cliente viene a pagar — crear nueva póliza"
                    >
                      + Nueva póliza
                    </a>
                  </div>
